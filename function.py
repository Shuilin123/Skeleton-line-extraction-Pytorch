...
def get_dataloaderV2(args):
    ...
    dataiter=iter(val_set);
    imgs,labels=next(dataiter);
    print("inage1 size is:",imgs.size());
    val_loader = DataLoader(val_set, batch_size=args.batch_size,
                            shuffle=False, num_workers=0)
    dataiter=iter(val_loader);
    imgs,labels=next(dataiter);
    print("inage2 size is:",imgs.size());
    # Save some samples of feeding to the neural network
    if args.sample_visualization:
...
def train(train_loader,net,criterion,optimizer,device):
    ...
    for batch_idx, (inputs, targets) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        targets=targets.unsqueeze(1);#2022.5.6
        loss = criterion(outputs, targets.float()) #old
        #loss = criterion(outputs, torch.squeeze(targets).long()) #2022.5.6
        loss.backward()
        optimizer.step()
