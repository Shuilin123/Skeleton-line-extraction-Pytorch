...
class Test():
    ...
    def inference(self, net):
        ...
        #print("self.pred_patches shape",predictions.shape);
        #self.pred_patches = np.expand_dims(predictions,axis=1)
        print("predictions's max is",predictions.max());
        print("predictions's min is",predictions.min());
        print("predictions's mean is",predictions.mean());
        self.pred_patches=predictions
    ...
    def evaluate(self):
        ...
        #print("y_true val is:",y_true)
        y=y_true;
        print("old y_true shape",y_true.shape);
        #print("old y_true type",y_true.dtype());
        ...
        print("new y_true shape",y_true.shape);
    ...
    def save_segmentation_result(self):
        ...
        self.save_img_path1 = join(self.save_img_path,'predict_img')
        if not os.path.exists(join(self.save_img_path1)):
            os.makedirs(self.save_img_path1)
        self.save_img_path2 = join(self.save_img_path,'label_img')
        if not os.path.exists(join(self.save_img_path2)):
            os.makedirs(self.save_img_path2)
        self.save_img_path3 = join(self.save_img_path,'binary_img')
        if not os.path.exists(join(self.save_img_path3)):
            os.makedirs(self.save_img_path3)
        if not os.path.exists(join(self.save_img_path)):
            os.makedirs(self.save_img_path)
        # self.test_imgs = my_PreProc(self.test_imgs) # Uncomment to save the pre processed image
        for i in range(self.test_imgs.shape[0]):
             #total_img,
            pred_res,binary,gt = concat_result(self.test_imgs[i],self.pred_imgs[i],self.test_masks[i])
            #save_img(total_img,join(self.save_img_path, "Result_"+img_name_list[i]+'.png'))
            save_img(pred_res,join(self.save_img_path1, "Result_"+img_name_list[i]+'.png'))
            save_img(gt,join(self.save_img_path2, "Result_"+img_name_list[i]+'.png'))
            save_img(binary,join(self.save_img_path3, "Result_"+img_name_list[i]+'.png'))
...
if __name__ == '__main__':
    ...
    net=UCTransNet(config_vit,n_channels=1,n_classes=1)
