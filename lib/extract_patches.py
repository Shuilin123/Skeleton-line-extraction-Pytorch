...
def load_data(data_path_list_file):
    ...
    for i in range(len(img_list)):
        ...
        if len(fov.shape)==3:
            fov = fov[:,:,0]
        print("i:",i)
        print("imgs1 shape is:",img.shape);
        print("imgs2 shape is:",np.expand_dims(img,0).shape);
    ...
    print("shape1 is:",imgs.shape);#[20,584,565,3]
    #imgs = np.transpose(imgs,(0,3,1,2))
    #imgs=np.unsqueeze(imgs,1);
    imgs=np.expand_dims(imgs,1);
    print("shape2 is:",imgs.shape);#[20,3,584,565]
