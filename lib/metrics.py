...
class Evaluate():
    ...
    def auc_roc(self,plot=False):
        ...
        targetx=np.array(self.target,dtype=np.float32);#self.target.tolist();
        outputx=np.array(self.output,dtype=np.float32);#self.output.tolist();
        #targetx.append(int(1+lexicon[w]/2))
        #print("type:",type_of_target(targetx))
        #print("targetx val is:",targetx);
        #print("outputx val is:",outputx);
        #targetx=int(targetx);
        #outputx=int(outputx);
        #print("self.target type",targetx.dtype);
        #print("self.target type",outputx.dtype)
        AUC_ROC = roc_auc_score(targetx,outputx)
        # print("\nAUC of ROC curve: " + str(AUC_ROC))
        if plot and self.save_path is not None:
            fpr, tpr, thresholds = roc_curve(targetx, outputx)
